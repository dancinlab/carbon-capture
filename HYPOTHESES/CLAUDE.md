...
Pre-register → falsify → run → verdict, for the carbon-capture (HEXA-CCUS) problem.
...
3. **Run deterministically** — the run script lives in `state/<hX>_<slug>_<date>/`,
   imports `tool/carbon_capture.py`, writes `result.json`, prints a verdict.
...
- Shared physics → `tool/carbon_capture.py`. Per-hypothesis run + result → `state/`.
